...
```{r  Biplot Grouped by Genus with legends }
...
```{r   Biplot Grouped by Genus without legends}

## Chunk: Biplot with Filtered Genus Ellipses (Legend Removed)

fig.cap="Figure 2a: Biplot Grouped by Genus Variable"
res.pca_filtered <- prcomp(foretnan_filtered, scale = TRUE) 
group_variable_filtered <- foret_filtered$genus
...
fviz_pca_biplot(res.pca_filtered, 
                ...
                label = "var", 
                repel = TRUE, 
                # Use a standard point shape (fixes unimplemented pch warnings)
                pointshape = 19, 
                ggtheme = theme_minimal()
) + 
...
# Ensure res.pca and foret$genus are defined correctly beforehand.
 fig.cap="Figure 2b: Biplot Grouped by Genus"
group_variable <- foret$genus
